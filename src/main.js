...
try {
  ...
  lastSubmitTxt = lastSubmitMatch ? lastSubmitMatch[7] : '';
  publishPerson = gitInfo.match(/(?<=user.name=)(.*)/g)[0];
} catch (error) {
  console.log('BuildTime Plugin Err', error)
}
const time = new Date().toLocaleString()
export const buildTime = `构建时间:${time};构建人:${publishPerson};分支:${branch};最后提交msg:${lastSubmitTxt.trim()};最后提交时间:${lastSubmitTime};最后提交commitId:${lastId};`
